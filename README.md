# Simple KCP File Transfer

简单的基于 KCP 协议的高速文件传输工具。

## ✨ 特性

- **KCP 协议**：基于 UDP 的可靠传输，比 TCP 更快，适合高延迟、高丢包网络
- **多线程下载**：8 线程并行分块下载，充分利用带宽
- **断点续传**：支持暂停/恢复，意外中断后可继续下载
- **完整性校验**：SHA256 校验确保文件完整无损
- **加密传输**：AES 加密保护数据安全
- **文件夹下载**：支持递归下载整个文件夹
- **跨平台**：支持 Windows、Linux（amd64/arm64）
- **双模式**：GUI 图形界面 + CLI 命令行

## 🏗️ 技术实现

```
┌─────────────────────────────────────────────────────────┐
│                      Application                        │
├─────────────────────────────────────────────────────────┤
│                    HTTP Protocol                        │
├─────────────────────────────────────────────────────────┤
│                  smux (Multiplexing)                    │
├─────────────────────────────────────────────────────────┤
│              KCP (Reliable UDP Transport)               │
├─────────────────────────────────────────────────────────┤
│                        UDP                              │
└─────────────────────────────────────────────────────────┘
```

- **传输层**：[KCP](https://github.com/skywind3000/kcp) - 快速可靠的 ARQ 协议
- **多路复用**：[smux](https://github.com/xtaci/smux) - 在单个 KCP 连接上复用多个流
- **应用层**：标准 HTTP 协议，支持 Range 请求实现分块下载
- **GUI 框架**：[Fyne](https://fyne.io/) - Go 语言跨平台 GUI 库

### KCP 优化配置

采用类似 kcptun `fast3` 模式的激进配置：
- 窗口大小：1024
- NoDelay 模式：启用
- 间隔：10ms
- 重传：2 次
- NC：1（无拥塞控制）

## 📦 安装

### 从 Release 下载

前往 [Releases](../../releases) 页面下载对应平台的预编译二进制文件。

### 从源码编译

```bash
# 克隆仓库
git clone https://github.com/CertStone/simpleKcpFileTransfer.git
cd simpleKcpFileTransfer

# 安装依赖
go mod tidy

# 编译当前平台的客户端
go build -o client/client ./client

# 编译当前平台的服务端
go build -o server/server ./server
```

#### 批量交叉编译

```bash
# Linux/macOS
./scripts/build.sh

# Windows PowerShell
.\scripts\build.ps1
```

## 🚀 使用方法

### 服务端

```bash
# 默认启动，监听 8080 端口，共享当前目录
./server

# 指定共享目录和端口
./server -p 8080 -d /path/to/share

# 指定自定义加密密钥
./server -p 8080 -d /path/to/share -key "my-secret-key"
```

参数说明：
- `-p`：监听端口（默认 8080）
- `-d`：共享目录（默认当前目录）
- `-key`：加密密钥（默认使用内置密钥，生产环境建议自定义）

### 客户端

#### GUI 模式（默认）

```bash
./client
```

1. 输入服务器地址（如 `192.168.1.100:8080`）
2. 点击 **Connect** 连接服务器
3. 浏览文件列表，选择文件或文件夹
4. 点击 **Download File** 或 **Download Folder** 开始下载

#### CLI 模式

```bash
# 下载单个文件
./client -cli -addr 192.168.1.100:8080 -url /video.mp4 -out ./downloads

# 指定并行数
./client -cli -addr 192.168.1.100:8080 -url /large.zip -workers 8

# 使用自定义密钥连接
./client -cli -addr 192.168.1.100:8080 -url /video.mp4 -key "my-secret-key"
```

参数说明：
- `-cli`：启用命令行模式
- `-addr`：服务器地址（默认 `127.0.0.1:8080`）
- `-url`：远程文件路径
- `-out`：保存目录（默认 `./download`）
- `-workers`：并行下载线程数（默认 8）
- `-key`：加密密钥（默认使用内置密钥）

## 📸 截图

![客户端 GUI 界面](./docs/screenshot1.png)

## ⚙️ 配置

### 加密密钥

可以通过命令行参数 `-key` 指定自定义密钥。

**服务端：**
```bash
# 使用自定义密钥启动
./server -key "your-custom-key"

# 使用默认密钥时会显示警告
./server
# [WARNING] Using default encryption key. For production use, please specify a custom key with -key parameter!
```

**客户端：**
```bash
# CLI 模式指定密钥
./client -cli -key "your-custom-key" -addr 192.168.1.100:8080 -url /file.zip

# GUI 模式指定密钥
./client -key "your-custom-key"
```

在 GUI 模式下，如果连接失败（可能是密钥不匹配），会自动弹出对话框让用户输入密钥。

默认密钥定义在 `common/kcp.go`：

```go
const (
    Salt       = "kcp-file-transfer"
    DefaultKey = "your-secret-key"
)
```

**生产环境建议使用 `-key` 参数指定自定义密钥！**

### 性能调优

在 `client/main.go` 中可调整：

```go
const (
    defaultWorkers   = 8              // 并行下载线程数
    defaultChunkSize = 4 * 1024 * 1024 // 每个分块大小 (4MB)
    maxParallelTasks = 3              // 最大同时下载任务数
)
```

## 🔧 开发

### 项目结构

```
simpleKcpFileTransfer/
├── client/
│   └── main.go          # 客户端（GUI + CLI）
├── server/
│   └── main.go          # 服务端
├── common/
│   └── kcp.go           # 共享的 KCP/smux 配置
├── scripts/
│   ├── build.sh         # Linux/macOS 编译脚本
│   └── build.ps1        # Windows 编译脚本
├── docs/
│   └── screenshot1.png  # 截图
├── go.mod
├── go.sum
└── README.md
```

### 依赖

- [fyne.io/fyne/v2](https://github.com/fyne-io/fyne) - 跨平台 GUI
- [github.com/xtaci/kcp-go/v5](https://github.com/xtaci/kcp-go) - KCP 实现
- [github.com/xtaci/smux](https://github.com/xtaci/smux) - 流多路复用
- [golang.org/x/crypto](https://golang.org/x/crypto) - 加密库

## 📄 License

MIT License

## 🙏 致谢

- [xtaci/kcp-go](https://github.com/xtaci/kcp-go) - 优秀的 KCP Go 实现
- [xtaci/kcptun](https://github.com/xtaci/kcptun) - KCP 隧道，本项目参考了其配置
- [fyne-io/fyne](https://github.com/fyne-io/fyne) - 简洁的 Go GUI 框架
- GPT-5.1-Codex-Max 与 Claude Opus 4.5 - 代码生成与优化
